...
const SidebarDesktop = ({
    ...
    <div className="hidden md:flex h-full z-sidebar relative">

        <div
            id="sidebar-desktop"
            className={clsx(
                "flex flex-col fixed top-[60px] bottom-4 left-0 transition-all duration-300 ease-out glass-panel border-l-0 rounded-r-2xl mr-4 z-sidebar",
                    ...
                    {!analysis && !isLoading && (
                        <div className="flex flex-col items-center justify-center text-center py-4 px-4 animate-in fade-in zoom-in duration-500 select-none opacity-80 mt-2">
                            ...
                            <div className="relative mb-2 group">
                                <div className="absolute inset-0 bg-guinda rounded-full blur-xl opacity-5 group-hover:opacity-10 transition-opacity duration-700"></div>
                                <div className="relative w-10 h-10 bg-gradient-to-tr from-white to-gray-50 rounded-xl shadow-sm border border-gray-100 flex items-center justify-center">
                                    <Icons.MapIcon className="h-5 w-5 text-guinda opacity-80" strokeWidth={1.5} />
                                </div>
                            </div>
                    ...
                    {analysis && !isLoading && (
                        <>
                            <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-md -mx-3 px-3 py-3 border-b border-gray-100 shadow-sm flex items-center justify-between mb-2 transition-all">
                                <span className="text-[11px] font-bold text-guinda uppercase tracking-wider flex items-center gap-2">
        ...
        <Tooltip content={isOpen ? 'Ocultar panel lateral' : 'Mostrar panel lateral'}>
            <button
                ...
                className={`
                    ...
                    focus:outline-none focus:ring-2 focus:ring-guinda focus:ring-offset-2
